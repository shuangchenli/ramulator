...
public:
    ...
    {
    	if(configs.contains("queue_size")) {
        readq.max = configs.get_int_value("queue_size");
        writeq.max = configs.get_int_value("queue_size");  // queue for write requests
        otherq.max = configs.get_int_value("queue_size");  // queue for all "other" requests (e.g., refresh)
    	}
    ...
    void tick()
    {
        ...
        if (pending.size()) {
          ...
          if (req.depart <= clk) {
            ...
            DBG(response_packets_buffer.size());
            pending.pop_front();
            DBGMSG(clk);
          }
        }
        ...
        DBG(channel->spec->command_name[(int)cmd]);
