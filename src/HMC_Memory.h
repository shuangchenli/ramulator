...
public:
    ...
    bool send(Request req)
    {
        ...
        DBG(req.addr_vec[int(HMC::Level::Vault)]);
        DBG(req.addr_vec[int(HMC::Level::BankGroup)]);
        DBG(req.addr_vec[int(HMC::Level::Bank)]);
        DBG(req.addr_vec[int(HMC::Level::Row)]);
        DBG(req.addr_vec[int(HMC::Level::Column)]);
        req.arrive_hmc = clk;
        ...
        if (packet.header.TAG.value == -1) {
          ...
          DBG(packet.header.TAG.value);
          return false;
        }

        // TODO support multiple stacks
        if(bypass_links) {
        	if(ctrls[req.addr_vec[0]]->receive(packet)) {
        	   if (req.type == Request::Type::READ) {
        ...
        if (packet.total_flits <= link->slave.available_space()) {
          ...
          return true;
        	...
        	DBG(packet.total_flits);
        	DBG(link->slave.available_space());
          return false;
        }
    }
    ...
    int pending_requests()
    {
        int reqs = 0;
        for (auto ctrl: ctrls)
            reqs += ctrl->readq.size() + ctrl->writeq.size() + ctrl->otherq.size() + ctrl->pending.size() + ctrl->response_packets_buffer.size();
        for (auto layer: logic_layers) {
        		for(auto link: layer->host_links)
        			reqs += link->master.output_buffer.size() + link->slave.input_buffer.size();
        		for(auto link: layer->pass_thru_links)
        			reqs += link->master.output_buffer.size() + link->slave.input_buffer.size();
        }
